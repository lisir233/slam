...
{
    sub_point_cloud_ = nh.subscribe("/filtered_points_no_ground", 10, &PclTestCore::point_cb, this);

    pub_remove_powerintensity_ = nh.advertise<sensor_msgs::PointCloud2>("/filtered_remove_powerintensity", 10);
...
void PclTestCore::remove_powerintensity_pt(double power_threshold,  pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                                   pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
    pcl::PassThrough<pcl::PointXYZI> pt;	// 创建滤波器对象
    pt.setInputCloud(in);				// 设置输入点云
    pt.setFilterFieldName("intensity");	//设置滤波所需字段z
    pt.setFilterLimits(0, 150);		//设置Z字段过滤范围
    pt.setFilterLimitsNegative(false);	//默认false，保留范围内的点云；true，保存范围外的点云
    pt.filter(*out);			//执行滤波，保存滤波后点云
}   
void PclTestCore::projection_3D_to_2D_pt(double power_threshold,  pcl::PointCloud<pcl::PointXYZI>::Ptr in,
                                   pcl::PointCloud<pcl::PointXYZI>::Ptr out)
{
...
void PclTestCore::point_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr)
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr current_pc_ptr(new pcl::PointCloud<pcl::PointXYZI>);
    pcl::fromROSMsg(*in_cloud_ptr, *current_pc_ptr);
    pcl::PointCloud<pcl::PointXYZI>::Ptr remove_powerintensity_ptr(new pcl::PointCloud<pcl::PointXYZI>);
    projection_3D_to_2D_pt(MIN_DISTANCE, current_pc_ptr, remove_powerintensity_ptr);

    publish_cloud(pub_remove_powerintensity_, remove_powerintensity_ptr, in_cloud_ptr->header);
}